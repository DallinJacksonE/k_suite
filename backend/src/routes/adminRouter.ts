...
export interface AdminRouterDeps {
  access?: MariaDbAccess;
  bucket?: MinIoBucketService;
}
...
export function createAdminRouter(deps: AdminRouterDeps = {}): Router {
  ...
  router.post('/products', asyncHandler(async (req, res) => {
    const product = await access.addProduct(readAdminCookie(req), req.body as CreateProductInput);
    res.status(201).json({ product });
  }));

  router.patch('/products/:productId', asyncHandler(async (req, res) => {
    const product = await access.editProduct(readAdminCookie(req), readParam(req, 'productId'), req.body as UpdateProductInput);
    res.json({ product });
  }));
  ...
  router.post('/blog/articles', asyncHandler(async (req, res) => {
    const article = await access.createBlogArticle(readAdminCookie(req), req.body as CreateBlogArticleInput);
    res.status(201).json({ article });
  }));

  router.patch('/blog/articles/:articleId', asyncHandler(async (req, res) => {
    const article = await access.updateBlogArticle(readAdminCookie(req), readParam(req, 'articleId'), req.body as UpdateBlogArticleInput);
    res.json({ article });
  }));
...
function parsePhotoCategory(value: string): PhotoCategory {
  if (value === 'product' || value === 'blog') return value;
