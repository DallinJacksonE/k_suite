...
import type { ApiEndpointDoc, BlogArticleInput, BlogArticleRecord, OrderRecord, ProductRecord, ServiceHealthReport, UpdateProductInput } from '../service/AdminApiService'
...
export function AdminDashboardView() {
  ...
  return (
    <AdminShell adminName={adminName} activeCategory={activeCategory} busy={busy} onSelect={setActiveCategory} onLogout={() => presenter.logout()}>
      ...
      {activeCategory === 'blog' ? <BlogDashboard articles={blogArticles} busy={busy} onCreate={(input: BlogArticleInput) => void presenter.createBlogArticle(input)} onUploadImage={(file) => presenter.uploadBlogImage(file)} onRefresh={() => void presenter.loadBlogArticles()} /> : null}
