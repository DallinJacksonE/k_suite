...
function ShippingConfirmation({ order, busy, onMarkShipped }: { order: OrderRecord; busy: boolean; onMarkShipped(orderId: string): void }) {
  ...
  return <><button type="button" onClick={() => openDialog(dialogId)} disabled={busy}>Mark shipped</button><dialog id={dialogId} className="shipping-confirmation-modal"><form method="dialog" className="shipping-confirmation-card"><div className="modal-header"><div><p className="eyebrow">Shipping check</p><h3>Mark order {order.orderId} shipped</h3></div><button type="button" className="secondary-button" onClick={() => closeDialog(dialogId)}>Close</button></div><section><h4>Client address</h4>{address ? <address>{formatAddress(address)}</address> : <p className="muted-text">No shipping address saved for this order.</p>}<label className="checkbox-label"><input type="checkbox" onChange={(event) => syncShippingConfirmation(event.currentTarget.form)} />Shipping label is accurate</label></section><section><h4>Plushies ordered</h4>{plushies.length ? <ul className="shipping-plushie-list">{plushies.map((item) => <li key={`${item.productId ?? item.title}-${item.selectedColor ?? ''}-${item.selectedSize ?? ''}`}><strong>{item.title}</strong><span>Qty {item.quantity ?? 1}</span><span>Color: {item.selectedColor || 'Not specified'}</span><span>Size: {item.selectedSize || 'Not specified'}</span><span>Notes: {item.clientInstructions || 'No notes'}</span></li>)}</ul> : <p className="muted-text">No plushie line items saved for this order.</p>}<label className="checkbox-label"><input type="checkbox" onChange={(event) => syncShippingConfirmation(event.currentTarget.form)} />Color and size are correct and Notes were considered</label></section><div className="button-row"><button type="button" data-shipping-confirm data-busy={busy ? 'true' : 'false'} disabled onClick={(event) => { closeDialog(dialogId); onMarkShipped(order.orderId); (event.currentTarget.form?.querySelectorAll('input[type="checkbox"]') ?? []).forEach((input) => { (input as HTMLInputElement).checked = false }); syncShippingConfirmation(event.currentTarget.form) }}>Mark Shipped</button></div></form></dialog></>
}
...
function syncShippingConfirmation(form: HTMLFormElement | null): void { const button = form?.querySelector<HTMLButtonElement>('[data-shipping-confirm]'); const checkboxes = [...(form?.querySelectorAll<HTMLInputElement>('input[type="checkbox"]') ?? [])]; if (button) button.disabled = button.dataset.busy === 'true' || checkboxes.length < 2 || checkboxes.some((input) => !input.checked) }
